    ...
    tone === 'good'
      ...
      : tone === 'bad'
        ...
        : tone === 'warn'
          ...
          : 'ring-line'
  return (
    <div className={`rounded-lg bg-surface-2 px-3 py-2.5 ring-1 ${ringClass}`}>
      <p className="eyebrow text-[10px]">{label}</p>
      <p className={`num mt-1 text-sm font-semibold ${toneClass}`}>{value}</p>
      {hint ? <p className="mt-0.5 text-[10px] text-ink-3">{hint}</p> : null}
    </div>
  )
}
    ...
    head === 'A' || head === 'B'
      ...
      : head === 'C'
        ...
        : head === 'D' || head === 'F'
          ...
          : 'bg-surface-3 text-ink-2 ring-line-strong'
  return (
    <span className="inline-flex items-center gap-1.5">
      <span
        className={`num inline-flex size-7 items-center justify-center rounded-md text-sm font-bold ring-1 ${tone}`}
        title={`FMP rating: ${letter ?? '—'}${score != null ? ` (${score}/5)` : ''}`}
      >
        {letter ?? '?'}
      </span>
      {score != null ? (
        <span className="num text-[11px] text-ink-2">{score}/5</span>
      ) : null}
    </span>
  )
}
  ...
  return (
    <section>
      ...
      <button
        ...
        className="mt-1.5 text-[11px] font-medium text-flame transition-colors duration-150 hover:text-ember outline-none focus-visible:ring-2 focus-visible:ring-ember/60 rounded"
      >
...
function SkeletonCard() {
  return <div className="skeleton h-[58px]" aria-hidden />
}
    ...
    <aside className="panel">
      <header className="flex items-start justify-between gap-3 border-b border-line px-4 py-3">
        <div className="min-w-0">
          ...
          {headerSubtitle ? (
            <p className="mt-0.5 truncate text-[11px] text-ink-3">{headerSubtitle}</p>
          ) : null}
        </div>
        <button
          type="button"
          onClick={() => setCollapsed((v) => !v)}
          className="shrink-0 rounded-md p-1 text-ink-3 transition-colors duration-200 hover:bg-surface-3 hover:text-ink outline-none focus-visible:ring-2 focus-visible:ring-ember/60"
      ...
      {collapsed ? null : (
        <div className="space-y-5 px-4 py-4">
          {error ? (
            <div className="rounded-lg border border-down/20 bg-down/10 p-3 text-xs text-down">
              <p className="font-medium">Couldn’t load fundamentals</p>
              <p className="mt-0.5 text-down/80">{error}</p>
            </div>
          ) : loading && !data ? (
            <>
            ...
            <>
              ...
              <Section title="Quality">
                ...
                <div className="rounded-lg bg-surface-2 px-3 py-2.5 ring-1 ring-line">
                  <p className="eyebrow text-[10px]">FMP Rating</p>
                  <div className="mt-1">
